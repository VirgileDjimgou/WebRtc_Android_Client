...
import com.android.gudana.chat.interface_listeners_menu.RecyclerTouchListener;
import com.android.gudana.chat.interface_listeners_menu.Toolbar_ActionMode_Callback;
...
public class ChatActivity extends AppCompatActivity {
    ...
    private int Lastmsg_Id; // extrem important because of the index of last messages on server file ...
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RecyclerViewMessages.setOnScrollListener(new RecyclerView.OnScrollListener() {

            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                super.onScrollStateChanged(recyclerView, newState);
                if(RecyclerViewMessages.SCROLL_STATE_DRAGGING==newState){
                    //fragProductLl.setVisibility(View.GONE);
                }
                if(RecyclerViewMessages.SCROLL_STATE_IDLE==newState){
            ...
            */
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                ...
                if (firstVisibleItem == 0) {
                    // check if we reached the top or bottom of the list
                    View v = RecyclerViewMessages.getChildAt(0);
                    int offset = (v == null) ? 0 : v.getTop();
                    if (offset < 2) {
                        // reached the top:
                        Log.d("ChatActivity", "first_message_history_lock=" + first_message_history_lock);
        ...
        back_bottom.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                RecyclerViewMessages.smoothScrollToPosition(adapter.getCount()-1);
            }
        });
        ...
        btnSend.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (!msg.isEmpty()) {
                    ...
                    String msg_uuid = UUID.randomUUID().toString();
                    Boolean on_server = false;
                    int message_id = 0;

                    final MessageItem msgItem = new MessageItem(message_id,
                            user_id, username,  msg, call_time,
                            MessageItem.TEXT_TYPE , on_server , msg_uuid);

                    new SendMessageTask(msgItem).execute();
        ...
        {
            int previousHeight = root.getRootView().getHeight() - root.getHeight() - RecyclerViewMessages.getHeight();

            @Override
            public void onGlobalLayout()
            {
                int height = root.getRootView().getHeight() - root.getHeight() - RecyclerViewMessages.getHeight();
                ...
                if(previousHeight != height)
                {
                    ...
                    else if(previousHeight < height)
                    {
                        ChatActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                RecyclerViewMessages.scrollToPosition(adapter.getCount() - 1);
                            }
                        });
    ...
    private class Init_after_OnCreate_Task extends AsyncTask<String, String, Void> {
        String Init_message;

        public Init_after_OnCreate_Task(String Init)
        {
          this.Init_message = Init;
        }
        ...
        protected Void doInBackground(String... args) {
            ...
            mDb = AppDatabase.getDatabase(getApplicationContext() , currentUserId+"chat");
            // Init Socket ....
            ...
            getAllmessage();
            System.out.println(" getting offline message  ...");
    ...
    // implement recyclerview Interface for click listeners   ......

    //Implement item click and long click over recycler view
    private void implementRecyclerViewClickListeners() {
        RecyclerViewMessages.addOnItemTouchListener(new RecyclerTouchListener(ChatActivity.this, RecyclerViewMessages, new RecyclerClick_Listener() {
            @Override
    ...
    private void onListItemSelect(int position) {
        ...
        //RecyclerViewMessages.set

        boolean hasCheckedItems = adapter.getSelectedCount() > 0;//Check if any items are already selected or not


        System.out.println("action for click listeners .... ");
        if (hasCheckedItems && mActionMode == null)
            // there are some selected items, start the actionMode
            mActionMode = ((AppCompatActivity) ChatActivity.this).
                    startSupportActionMode(new Toolbar_ActionMode_Callback(ChatActivity.this,
                            ChatActivity.this, adapter, adapter.dataSet, false));
    ...
    public void deleteRows() {
        ...
        for (int i = (selected.size() - 1); i >= 0; i--) {
            if (selected.valueAt(i)) {
                ...
                adapter.dataSet.remove(selected.keyAt(i));
                adapter.notifyDataSetChanged();//notify adapter

            }
        }
        Toast.makeText(ChatActivity.this, selected.size() + " item deleted.", Toast.LENGTH_SHORT).show();//Show Toast
    ...
    public void getAllmessage(){
        ...
        // get all message of this room ...
        // List<message> data = message_db.getAllMessage(Integer.toString(room_id), room_uid);
        offline_msg = DatabaseInitializer.load_All_Message(mDb);

        if(offline_msg.size() > 0){

            // get last MessageObjet on the list of Array  ...
            MessageItem LastMessageObject = offline_msg.get(offline_msg.size() - 1);
            // set last index  to request the unread messages
            Lastmsg_Id = LastMessageObject.getID();
            ChatActivity.this.runOnUiThread(new Runnable() {
                @Override
                public void run() {

                    // problaby load to parser before addd on  Recycler view
                    adapter.addItems(offline_msg);
                    RecyclerViewMessages.scrollToPosition(adapter.getCount() -1);
                }
            });


            //  send a request to get offline messages  ... also unread messages  ...
            ...
            try {
                ...
                if(adapter.getCount() > 0) {
                    ...
                    json.put("after_id_message", Lastmsg_Id-1); // get last message - 3
    ...
    public void parse_offline_message(JSONObject json) {
        final int msg_user_id, message_id , msg_type;
        final String msg_username, message_contents, datetimeutc , msg_uuid;
        final Boolean on_server;
        try {
            ...
            msg_type = json.getInt("msg_type");
            on_server = json.getBoolean("on_server");
            msg_uuid = json.getString("msg_uuid");
            ...
            if(user_id == msg_user_id ) {
                ...
                ChatActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        //MessageItem msg_item = (MessageItem) adapter.getItem(not_on_server_indices.get(0));
                        ...
                        RecyclerViewMessages.setAdapter(RecyclerViewMessages.getAdapter());
                ...
                final MessageItem msgItem = new MessageItem(message_id,
                        ...
                        message_contents,
                        datetimeutc, msg_type , on_server , msg_uuid);
...
*/
    ...
    private final Emitter.Listener onMessageReceive = new Emitter.Listener() {
        ...
        public void call(Object... args) {
            ...
            try {
                ...
                // your messages .... get the uuid and update the indices on recyclerview ...
                if(user_id == msg_user_id) {
                    /** TODO: remove assumption that messages are received in order
                     * Proper way is to sort messages by their IDs in ascending order
                     **/
                    ...
                    ChatActivity.this.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            final MessageItem msgItem =
                                    new MessageItem(message_id, user_id,
                                            msg_username, message_contents,
                                            datetimeutc , msg_type ,  true , msg_uuid);
                    ...
                    // other user message  .... update  Rycyclerview

                    final MessageItem msgItem = new MessageItem(message_id, user_id, msg_username,
                            message_contents, datetimeutc , msg_type , true , msg_uuid);

                    ChatActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {

                            // save message for other user offline  ....
                            // save message  on local  database  for offline use  ...
                            // new Save_offline_MessageTask (json).execute();
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    try {
                        ...
                        if(message_id > Lastmsg_Id){
                            ...
                            if(user_id == msg_user_id ) {
                                //  your message  if the user_id are the same   ...
                                /** TODO: remove assumption that messages are received in order
                                 * Proper way is to sort messages by their IDs in ascending order
                                 **/
                                ...
                                ChatActivity.this.runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        // move it to the end of the list.
                                        // adapter.moveItemToEndOfList(not_on_server_indices.get(0));
                                        //on_server = true; // to tell that we can update the ui and the  the db   ..
                                        final MessageItem msgItem =
                                                new MessageItem(message_id, user_id,
                                                        msg_username, message_contents,
                                                        datetimeutc , msg_type ,  true , msg_uuid);
    ...
    private class SendMessageTask extends AsyncTask<String, String, Void> {
        ...
        public SendMessageTask(MessageItem Msg_Item) {
        ...
        protected Void doInBackground(String... args) {
            ...
            */
            ...
            try {
                ...
                inputJson.put("msg_type", this.Msg_Item.getMsg_type());
                inputJson.put("on_server" , this.Msg_Item.isOn_server());
                inputJson.put("username" , this.Msg_Item.getUsername());
                inputJson.put("message" , this.Msg_Item.getMessage());
                inputJson.put("msg_uuid", this.Msg_Item.getMsg_uuid());
            ...
            // save message for currentuser offline  ....
            // save message  on local  database  for offline use  ...
            // new Save_offline_MessageTask (inputJson).execute();
            // persist message Asnycronly offline  ...
            DatabaseInitializer.insert_new_message_Async(mDb , this.Msg_Item);
        ...
        protected void onPostExecute(Void a) {
            ...
            //final MessageItem msgItem = new MessageItem(user_id, username, message_contents , msg_type);
            // add message on Recyclerview  ...
    ...
    private class Save_offline_MessageTask extends AsyncTask<String, String, Void> {
        ...
        protected Void doInBackground(String... args) {
            ...
            try {
                // save data offline with room persistence librariy  ..
                /*
    ...
    // udate offline message  and save  using Room database

    private class Update_offline_MessageTask extends AsyncTask<String, String, Void> {
        private final JSONObject message_contents;

        public Update_offline_MessageTask(JSONObject message) {
            this.message_contents = message;
        }
                ...
                if(resultCode == Activity.RESULT_OK){
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            /*
                            MessageItem msgItem =
                                    new MessageItem(user_id, username, message_contents ,
                                            MessageItem.LIVE_LOCATION , on_server , msg_uuid);
            ...
            case FilePickerConst.REQUEST_CODE_DOC:
                if (resultCode == Activity.RESULT_OK && data != null) {
                    ...
                    for (String object: docPaths) {
                        ...
                        /*
                        final MessageItem msgItem = new MessageItem(0, user_id,
                                "###upload_task_to_server###", msg_uuid,
                                call_time.toString(), file_local , file_local , true, MessageItem.DOC_TYPE ,on_server , msg_uuid);

                         */
                        ChatActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                //adapter.addItem_and_uploadtask(ChatActivity.this , msgItem);
                                    ...
                                    .subscribe(new Consumer<File>() {
                                        ...
                                        public void accept(final File file_compressed) {
                                            ...
                                            final MessageItem msgItem = new MessageItem(0, user_id,
                                                    ...
                                                    file_compressed , true , MessageItem.IMAGE_TYPE , on_server , msg_uuid);
    ...
    public  void images_upload_images_to_server(String  Url_media){
        ...
        try{
            ...
            final MessageItem msgItem = new MessageItem(message_id,
                    user_id, username,  msg, call_time,
                    MessageItem.IMAGE_TYPE , on_server , msg_uuid);
            new SendMessageTask(msgItem).execute();
    ...
    public void upload_messagev_voice(String uri){
        ...
        try{
            ...
            final MessageItem msgItem = new MessageItem(message_id,
                    user_id, username,  msg, call_time,
                    MessageItem.AUDIO_TYPE , on_server , msg_uuid);
            new SendMessageTask(msgItem).execute();
    ...
    private void voice_upload_images_to_firebase(File mAudioFile_to_send){
        ...
        final MessageItem msgItem = new MessageItem(0, user_id,
                "###upload_task_to_server###", msg_uuid, call_time.toString(), mAudioFile_to_send ,
                mAudioFile_to_send , true , MessageItem.AUDIO_TYPE , on_server , msg_uuid);
    ...
    public void doc_file__upload_to_Server(String uri){
        ...
        try{
            ...
            final MessageItem msgItem = new MessageItem(message_id,
                    user_id, username,  msg, call_time,
                    MessageItem.DOC_TYPE , on_server , msg_uuid);
